...
import com.appme.filepicker.adapter.DirectoryAdapter;
import com.appme.filepicker.adapter.PathAdapter;
import com.appme.filepicker.adapter.events.DirClickListener;
import com.appme.filepicker.adapter.events.FileClickListener;
import com.appme.filepicker.adapter.events.PathClickListener;
import com.appme.filepicker.data.FileInfo;
import com.appme.filepicker.thread.ScannerThread;
import com.appme.filepicker.thread.event.ScanListener;
import com.appme.filepicker.utils.FileUtil;
import com.appme.filepicker.utils.ViewUtil;
...
//        }
        ...
        directoryAdapter.setDirClickListener(new DirClickListener(){
            @Override
            public void onDirClicked(FileInfo info){
                onDirectoryClicked(info);
            }
        });
...
//        dirRecyclerView.setLayoutManager(new LinearLayoutManager(this));
    ...
    private void onDirectoryClicked(FileInfo fileInfo) {
        ...
        scannerThread.setScanListener(new ScanListener() {
            @Override
            public void onScanComplete(final FileInfo[] fileInfos) {
    ...
    private void onPathDirectoryClicked(final int index) {
        final File file = new File(directoriesPath.get(index));
        if (textView.getVisibility() == View.VISIBLE)
            textView.setVisibility(View.GONE);
        if (scannerThread != null && scannerThread.isAlive())
            scannerThread.setIgnoreListener(true);
