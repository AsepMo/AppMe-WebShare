...
public class ProgressAdapterHelper implements Downloader.OnDownloaderChangedListener {
    ...
    private final WebChatServer server;
    private final ArrayList<FileData> files;
    private final Object mutex;
    private OnNoFileTransfersListener onNoFileTransfersListener;

    public ProgressAdapterHelper(ApplicationActivity activity, WebChatServer server) {
...
//    }
    ...
    @Override
    public void onAdded(final Downloader.FileDownloader fileDownloader) {
            activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    synchronized (mutex) {
    ...
    @Override
    public void onRemoved(final Downloader.FileDownloader fileDownloader) {
            activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    synchronized (mutex) {
